...
export function scenarioMetrics(config, s) {
  ...
  const rows = [
    ...
    ...(s.traffic ? [{ group: 'Design', label: 'Peak requests / second', value: s.traffic.requestsPerSec, format: (v) => v.toFixed(2), better: null }] : []),
    { group: 'Design', label: inference ? 'Concurrent streams' : 'Micro-batch', value: inference ? (s.traffic ? s.traffic.concurrency : config.concurrency) : config.microBatchSize, format: (v) => v.toLocaleString(), better: null },
